...
class GlyphBatch {
    ...
    private var bmTexture:RectangleTexture;
    ...
    public function createProgram(ctx: Context3D) {
        ...
        bmTexture = ctx.createRectangleTexture(bm.width, bm.height, BGRA, false);
        bmTexture.uploadFromBitmapData(bm);
    ...
    public function render(ctx: Context3D) 
    {
        ...
        ctx.setTextureAt(0, bmTexture);
        ctx.setSamplerStateAt(0, CLAMP, NEAREST, MIPNONE);

        var uidx = program.getConstantIndex("uMatrix");
        ctx.setProgramConstantsFromMatrix(VERTEX, uidx, bitmapRenderTransform, false);
        ctx.setVertexBufferAt(program.getAttributeIndex("aPosition"), vBuffer, 0, FLOAT_2);
        ctx.setVertexBufferAt(program.getAttributeIndex("aTexCoord"), vBuffer, 2, FLOAT_2);
